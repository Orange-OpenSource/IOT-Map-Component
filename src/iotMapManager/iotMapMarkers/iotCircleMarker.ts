...
export class iotCircleMarker {
  ...
  //---------------------------------------
  // getSvg
  // return svg for circle marker according to parameter
  //---------------------------------------
  borderColor : string = "";
  svgCenter : string = "";
  svgIcon : string = "";
  ...
  getSvg (marker) {
    ...
    // border color
    this.borderColor = (marker.selected) ? marker.color : config.markers.circles.borderColor;

    // center color
    this.svgCenter = (marker.plain) ? `<path fill="` + marker.color + `" ` + commonSvg.circle_center + `/>`
    ...
    // marker anchored
    let svgToReturn = (marker.anchored) ? this.getCircleAnchoredSvg(marker) : this.getCircleSvg(marker);
  ...
  //---------------------------------------
  // getCircleAnchoredSvg
  // return svg for circle marker with anchor at right dimension
  //---------------------------------------
  getCircleAnchoredSvg(marker) {
    let border = `<path fill="` + this.borderColor +`" ` + commonSvg.circle_anchored_border + `/>`;
    let innerColor = `<path fill="` + marker.color + `" ` + commonSvg.circle_anchored_inner_color + `/>`;

    return border + innerColor + this.svgCenter + this.svgIcon + this.svgLabel;
  }


  //---------------------------------------
  // getCircleSvg
  // return svg for circle marker with no anchor at right dimension
  //---------------------------------------
  getCircleSvg(marker) {
    let border = `<path fill="` + this.borderColor +`" ` + commonSvg.circle_border + `/>`;
    let inner_color = `<path fill="`+ marker.color + `" ` + commonSvg.circle_inner_color + `/>`;

    return border + inner_color + this.svgCenter + this.svgLabel + this.svgIcon;
  }
