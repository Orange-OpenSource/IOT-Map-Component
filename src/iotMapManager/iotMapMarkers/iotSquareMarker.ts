...
export class IotSquareMarker {
  getSvg (marker, selected = false){
    ...
    // border color
    const borderColor = (selected) ? marker.shape.color : config.markers.square.borderColor;

    // background color
    const backGroundColor = (marker.shape.shape == 'poi' || marker.shape.plain) ? marker.shape.color : config.markers.square.backgroundColor;

    // inner
    if (marker.inner) {
      // todo : à revoir pour bien centrer
      svgInnerDesign = (marker.inner.label) ? `<text font-size='250' fill='` + marker.inner.color + `'
                                                stroke='#003366'
    ...
    // build shape
    const border = `<path fill='` + borderColor + `' ` + ((marker.shape.anchored) ? commonSvg.sqr_anchored_border : commonSvg.sqr_border) + `/>`;
    const inner_color = `<path fill='`+ marker.shape.color + `' ` + ((marker.shape.anchored) ? commonSvg.sqr_anchored_inner_color : commonSvg.sqr_inner_color) + `/>`;
    const inner_shape = `<path fill='`+ backGroundColor + `' ` + ((marker.shape.anchored) ? commonSvg.sqr_anchored_inner_shape : commonSvg.sqr_inner_shape) + `/>`;

    // result
    return  `<svg xmlns='http://www.w3.org/2000/svg' width='`+ width + `' height='`+ height + `' viewBox='0 0 450 545'>`
            + border + inner_color + inner_shape + svgInnerDesign
            + `</svg>`;
  }
}
